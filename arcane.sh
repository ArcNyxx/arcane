...
rd pacman -S teensy_loader_cli

# downgrade avr-gcc to 8.3.0 if necessary
[ "$(avr-gcc --version | sed '1q' | cut '-d ' -f3 | cut -d. -f1)" -gt 8 ] &&
	rd pacman -U https://archive.archlinux.org/packages/a/avr-gcc/avr-gcc-8.3.0-1-x86_64.pkg.tar.xz

# install flashing udev rules
rd qmk_firmware/util/udev/50-qmk.rules /etc/udev/rules.d/
